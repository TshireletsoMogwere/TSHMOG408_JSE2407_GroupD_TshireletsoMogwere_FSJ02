...
*/
export default function ImageCarousel({ images, thumbnail }) { //takes two props, thumbnail will be used as placeholder
    const [currentIndex, setCurrentIndex] = useState(0); //image displays initially

    if (!images || images.length === 0 || !thumbnail || thumbnail.length === 0) { //checks for empty or undefined arrays
      return (
...
/**
 * Updates the index state by incrementing or decrementing the current index.
 *
 * @param {Object} props - The component's props.
 * @param {Array} props.images - An array of image URLs to be displayed in the carousel.
 * @param {Array} props.thumbnail - An array of thumbnail URLs to be used as placeholders for the images.
...
const handleNext = () => {
  setCurrentIndex((prevIndex) => (prevIndex + 1) % images.length); {/* modulo operator ensures wrapping back to 0 */}
};
  ...
  setCurrentIndex((prevIndex) =>
    ...
    return (
      <div className="relative">
        <Image
          src={images[currentIndex]} //displays image of current index
          alt="Product image"
          width={150}
          height={150}
        ...
        {images.length > 1 && ( //check for more than one image
          <>
            <button
              onClick={handlePrev}
